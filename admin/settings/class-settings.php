...
* @subpackage bobs-custom-login/settings/admin
* @since      1.0.0
*/

if ( ! class_exists( 'bobs_Settings_Bcl' ) ) {

    class bobs_Settings_Bcl {
        ...
        $empty = true,
        $notices = array();
        const text_domain = 'bobs-custom-login';
        ...
        {
            wp_enqueue_media();
            wp_enqueue_script( 'bobs-settings-js', plugins_url( 'settings.js' , __FILE__ ), array( 'jquery', 'wp-color-picker' ) );
            wp_localize_script( 'bobs-settings-js', 'ajax', array(
                'url' => admin_url( 'admin-ajax.php' ),
                'spinner' => admin_url( 'images/spinner.gif' )
            ) );
            wp_enqueue_style( 'bobs-settings-css', plugins_url( 'settings.css' , __FILE__ ) );
...
<form action="options.php" method="POST" enctype="multipart/form-data" class="wrap">
    ...
    <div class="bobs-settings-tabs"></div>
    <?php }
...
<?php }
        ...
        public function do_section( $args )
        {
            ...
            global $wp_version;
            if ($wp_version >= 4.4) {
                echo "<input name='{$id}[{$this->page}_setting]' type='hidden' value='{$id}' class='bobs-settings-section' />";
            } else {
                echo "<input name='{$id}[{$this->page}_setting]' type='hidden' value='{$id}' class='bobs-settings-section-ante' />";
            }
//            echo "<input name='{$id}[{$this->page}_setting]' type='hidden' value='{$id}' class='bobs-settings-section' />";
        ...
        public static function do_field( $args )
        {
            ...
            switch ( $type )
            {
                ...
                case 'media':
                echo "<fieldset class='bobs-settings-media' id='{$id}'><input {$attrs} type='hidden' value='{$value}' />";
                echo "<p><a class='button button-large bobs-select-media' title='{$label}'>" . sprintf( __( 'Select %s', self::text_domain ), $label ) . "</a> ";
                echo "<a class='button button-small bobs-remove-media' title='{$label}'>" . sprintf( __( 'Remove %s', self::text_domain ), $label ) . "</a></p>";
                if ( $value ) {
                ...
                echo "<p class='bobs-settings-action'><input {$attrs} id='{$id}' type='button' class='button button-large' value='{$label}' /></p>{$desc}";
                break;

                case 'color':
                $v = esc_attr( $value );
                echo "<input {$attrs} id='{$id}' type='text' value='{$v}' class='bobs-settings-color' />{$desc}";
                break;
                ...
                foreach ( $this->settings[$setting]['fields'] as $name => $field ) {
                    ...
                    if ( $field['sanitize'] ) {
                        ...
                        switch ( $field['type'] )
                        {
                            ...
                            case 'textarea':
                            $text = '';
                            $nl = "bobs-SETTINGS-NEW-LINE";
                            $tb = "bobs-SETTINGS-TABULATION";
        ...
        {
            $bobs_settings = plugin_basename( __FILE__ );
            $active_plugins = get_option( 'active_plugins' );
            if ( $order = array_search( $bobs_settings, $active_plugins ) ) {
                array_splice( $active_plugins, $order, 1 );
                array_unshift( $active_plugins, $bobs_settings );
                update_option( 'active_plugins', $active_plugins );
            }
        }
    }
    add_action( 'activated_plugin', array( 'bobs_Settings_Bcl', 'plugin_priority' ) );
    ...
    function get_setting_bcl( $setting, $option = false )
    {
        ...
        if ( is_array( $setting ) ) {
            if ( $option ) {
                return isset( $setting[$option] ) ? bobs_Settings_Bcl::parse_multi( $setting[$option] ) : false;
            }
            foreach ( $setting as $k => $v ) {
                $setting[$k] = bobs_Settings_Bcl::parse_multi( $v );
            }
            return $setting;
        }
    ...
    {
        return new bobs_Settings_Bcl( $page, $title, $menu, $settings, $args );
    }

jQuery(document).ready(function ($) {
    ...
    var form = $('form.wrap'),
        ...
        tabs = $('.bobs-settings-tabs', form),
        current = parseInt(sessionStorage.getItem(page + '_current_tab'), 10) || 0;
    $('.bobs-settings-section', form).each(function (i, el) {
        var setting = $(el).val(),
            title = $(el).prev('h2'),
        ...
        if (tabs.length && title.length) {
            section.addClass('bobs-settings-tab').hide();
            title.appendTo(tabs).click(function (e) {
                e.preventDefault();
                if (!title.hasClass('active')) {
                    $('.bobs-settings-tab.active', form).fadeOut('fast', function () {
    ...
    $('.bobs-settings-section-ante', form).each(function (i, el) {
        var setting = $(el).val(),
            title = $(el).prev('h3'),
            section = $('<div>').attr('id', page + '_' + setting);
        $(el).nextAll().each(function () {
    ...
    $('.bobs-settings-media', form).each(function () {
        var frame,
            select = $('.bobs-select-media', this),
            remove = $('.bobs-remove-media', this),
            input = $('input', this),
            preview = $('img', this),
    ...
    $('.bobs-settings-action', form).each(function () {
    ...
    $('.bobs-settings-color').wpColorPicker();
});
